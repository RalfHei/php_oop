<div class="bg-white pb-4 px-4 rounded-md w-full max-w-screen-xl">
    <div class="flex justify-between w-full pt-6 ">
        <p class="ml-3 font-bold text-gray-700 text-lg"><?php echo $lang['tableTitle'] ?></p>

    </div>
    <div class="grid lg:grid-cols-8 w-full">
        <div class="col-span-2">
            <form method="GET" action="/admin" class="">
        ...
        <div class="flex justify-center">
            <a class="flex px-4 items-center justify-center font-bold 
    text-sm text-indigo-500 hover:text-indigo-300" href="/admin"><?php echo $lang['clear'] ?></a>
            <a class="flex items-center justify-center font-bold 
    text-sm text-indigo-500 hover:text-indigo-300" href="/register">
                <?php echo $lang['newUser'] ?>
            </a>
        </div>
    </div>
            ...
            <table class="table-auto border-collapse w-full">
                <thead>
                    <tr class="rounded-lg text-sm font-medium text-gray-700 text-left">
                        <th class="px-4 py-2 bg-indigo-200">Email</th>
                        ...
                        <th class="hidden md:table-cell px-4 py-2 bg-indigo-200"><?php echo $lang['tableRights'] ?></th>
                ...
                <tbody class="text-md font-normal text-gray-700">
                    <?php
                    if (!empty($user)) : foreach ($user as $use) { ?>
                            <tr class="hover:bg-gray-100 border-b border-gray-200 py-10">
                                <td class="px-4 py-2 " value="<?php echo $use['email'] ?>" id="inline-full-name" type="text" name="email"><?php echo $use['email'] ?></td>
                                <td class="px-4 py-2 hidden md:table-cell"><?php echo $use['added']; ?></td>
                                <td class="hidden md:table-cell px-4 py-2 "><?php echo $use['edited']; ?></td>
                                <td class="hidden md:table-cell px-4 py-2 "><?php echo $use['rights']; ?></td>
                                <td class="px-4 py-2 text-indigo-500 font-semibold "><a href="edit?user=<?php echo $use['id']; ?>"><?php echo $lang['editPost'] ?></a><a class="pl-4" href="delete?user=<?php echo $use['id']; ?>"><?php echo $lang['delete'] ?></a></td>
